...
RSpec.describe Conversation, type: :model do
  ...
  describe '.search_by_title' do
    ...
    it 'returns conversations matching the query (case-insensitive)' do # rubocop:disable RSpec/MultipleExpectations
      results = described_class.search_by_title('standup')
      expect(results).to include(matching)
      expect(results).not_to include(non_matching)
    end

    it 'returns partial matches' do
      expect(described_class.search_by_title('team')).to include(matching)
    end

    it 'returns none for blank query' do # rubocop:disable RSpec/MultipleExpectations
      expect(described_class.search_by_title('')).to be_empty
      expect(described_class.search_by_title(nil)).to be_empty
    end

    it 'sanitizes % in search input' do
      special = create(:conversation, title: '100% Complete')
      expect(described_class.search_by_title('100%')).to include(special)
    end

    it 'sanitizes _ in search input' do
      special = create(:conversation, title: 'file_name_test')
      expect(described_class.search_by_title('file_n')).to include(special)
    end
  end
end
